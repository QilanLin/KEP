...
Our framework makes three key contributions: (1) a \textit{differential testing methodology} that discovered 519 performance bugs across three major provers (E Prover, cvc5, Z3), representing timeouts, errors, and performance degradation; (2) an \textit{AST-based mutation fuzzer} for Isabelle theories with 10 intelligent mutation operators specifically designed for testing Sledgehammer integration; and (3) a \textit{two-phase workflow} where Mirabelle serves as ground truth for training a pattern-based oracle, achieving perfect alignment on the 130-mutation training set.

Experimental evaluation reveals where reliability issues manifest in the Sledgehammer ecosystem: differential testing exposed substantial volatility in underlying provers (519 performance bugs), while our AST-based fuzzing campaign with 130 mutations (validated by Mirabelle) suggests the integration layer handles these tested cases robustly. This provides insights for where reliability engineering efforts could focus.
...
We also developed a complete AST-based fuzzer specifically designed for testing Sledgehammer integration through Isabelle theory mutation. Unlike existing fuzzing tools that operate on bytes or tokens without understanding structure, our fuzzer parses Isabelle theories to extract lemmas and their components, then applies grammar-aware transformations that maintain syntactic validity. The fuzzer implements ten distinct mutation operators targeting different aspects of the integration layer, including quantifier transformations, logical operator modifications, term reordering, and proof method variations. Through three fuzzing campaigns of increasing scale, we generated and tested 130 mutations across 10 carefully selected seed theories, demonstrating that our fuzzer can methodically explore the input space while maintaining syntactic validity.

To ensure accurate bug detection, we developed a pattern-based oracle through a two-phase workflow where Mirabelle serves as ground truth. The oracle employs four-layer filtering logic that examines Isabelle's output for success indicators, distinguishes critical errors from informational messages, filters theory-level errors from integration bugs, and identifies Sledgehammer-specific interface issues. Through iterative refinement guided by Mirabelle feedback on the 130-mutation dataset, \textbf{the oracle achieved perfect alignment (0\% false positives on the training set)}, demonstrating successful learning of Mirabelle's judgment criteria.

Finally, our \textbf{systematic testing campaign} provides \textbf{fuzzing-based evidence} of Sledgehammer's integration robustness \textbf{within the tested mutation space}. Through 130 diverse mutations validated by Mirabelle, \textbf{no integration failures were observed}, successfully handling all syntactically valid structural transformations. This stands in stark contrast to the backend provers (519 performance failures), isolating reliability concerns to the computational layer rather than the interface implementation. The complete agreement between our custom oracle and Mirabelle on all test cases strengthens this conclusion, demonstrating that the observed resilience is not an artifact of oracle inaccuracy. This empirical finding has practical implications for resource allocation in quality assurance: testing and improvement efforts should focus on underlying prover performance rather than interface reliability.
...
To eliminate false positives, we developed a two-phase approach. The first phase employs a custom oracle for rapid screening of all test cases. This oracle is designed for high sensitivity, deliberately erring on the side of caution by flagging any potentially suspicious behavior for further investigation. While this approach may initially include false positives, it ensures that no genuine bugs are missed during the initial screening phase. The oracle achieves a throughput of approximately 32 tests per minute (31.7 measured), enabling rapid exploration of large test suites. This speed advantage is crucial for iterative development and large-scale fuzzing campaigns where testing thousands of mutations is necessary.

The second phase employs Mirabelle (Bulwahn \& Krauss, 2011), Isabelle's official testing tool specifically designed for evaluating automated proof tools like Sledgehammer. Mirabelle serves as our independent validator, providing authoritative judgments on whether flagged cases represent genuine bugs. While Mirabelle is slower than our custom oracle, its official status makes it the ideal validation mechanism. This two-phase approach combines the speed advantages of custom oracle screening with the reliability of official tool validation, achieving both high throughput on the tested set.

\subsubsection{Oracle Design}

Our oracle employs pattern-based classification with four-layer filtering logic. The analysis begins by examining Isabelle's output for success indicators such as ``Finished'' markers that indicate successful execution, preventing misclassification of warnings as errors. The oracle then performs critical error detection, distinguishing genuine critical errors marked with ``*** Error'' or ``*** Exception'' prefixes from informational messages or warnings. The analysis further filters theory-level errors, separating issues in the input theory file (such as syntax errors, type mismatches, or undefined references) from genuine integration bugs. Only errors occurring during Sledgehammer's interaction with external provers are classified as integration bugs. Finally, the oracle implements interface issue detection, specifically identifying Sledgehammer-related problems through patterns indicating TPTP encoding errors, prover communication failures, or proof reconstruction issues.

The oracle was refined through iterative alignment with Mirabelle as ground truth. On the 130-mutation training set, this approach achieved perfect alignment with Mirabelle with no false positives observed. This alignment is critical for meaningful fuzzing campaigns, as false positives would waste developer time and reduce confidence in reported bugs.
...
The codebase is organized around core components that handle different aspects of the testing workflow. Oracle modules handle bug detection: the crash and timeout detection oracle monitors prover execution for abnormal termination and timeout violations, while the differential oracle performs multi-prover comparison by parsing and normalizing outputs from different provers to detect result inconsistencies. The theory mutation engine parses Isabelle theory files and applies mutation operators while tracking validity, enabling structured exploration of the input space.

The fuzzing campaign orchestrator coordinates the complete automated workflow, managing mutation generation, test execution, bug detection, and statistical reporting. Integration bug detection is handled by a specialized oracle implementing the multi-layered contextual analysis logic described in Section 3.3. The verification module integrates with Mirabelle for official validation, providing the second phase of our two-phase verification approach that ensures zero false positives.
...
\begin{table}[h]
\centering
\caption{Fuzzing Campaign Resilience Metrics}
\label{tab:fuzzing}
\begin{tabular}{lrrrr}
\toprule
\textbf{Campaign} & \textbf{Seeds} & \textbf{Mutations} & \textbf{Time (min)} & \textbf{Pass Rate} \\
\midrule
Quick Test & 5 & 6 & 0.3 & 100\% \\
Medium Scale & 5 & 19 & 0.6 & 100\% \\
Large Scale & 10 & 105 & 3.3 & 100\% \\
\midrule
\textbf{Total} & \textbf{10} & \textbf{130} & \textbf{4.1} & \textbf{100\%} \\
\bottomrule
\end{tabular}
\end{table}

The fuzzing campaigns demonstrated excellent performance characteristics. Our framework achieved a throughput of approximately 31.7 mutations per minute, enabling rapid exploration of the input space while maintaining thorough testing of each mutation. The average test time of 1.9 seconds per mutation (computed as 4.1 minutes for 130 mutations) reflects the efficient implementation of our oracle and the overhead of invoking Isabelle for each test case. This performance is sufficient for practical continuous integration scenarios where hundreds or thousands of mutations might need to be tested regularly.

Across all campaigns, our mutation engine successfully employed 8 out of 10 available mutation operators, with the remaining 2 operators not triggered due to structural characteristics of the seed theories (for example, some operators require specific syntactic patterns that were not present in all seeds). This high operator utilization rate indicates that our seed selection was effective in covering diverse Isabelle features. Most importantly, no false positives were observed throughout all campaigns, confirmed by complete alignment with Mirabelle validation.

\subsubsection{Oracle-Mirabelle Alignment}

\begin{table}[h]
\centering
\caption{Oracle Training Results (measured on 130-mutation training set)}
\label{tab:oracle}
\begin{tabular}{lc}
\toprule
\textbf{Metric} & \textbf{Achieved} \\
\midrule
False Positive Rate & 0\% (training set) \\
Mirabelle Alignment & 100\% \\
Testing Throughput & $\approx$31.7 tests/minute \\
Average Test Time & 1.9s/test \\
\bottomrule
\end{tabular}
\end{table}

The results demonstrate that our two-phase approach successfully balances speed and alignment with official tool judgments. The oracle achieves a throughput of approximately 31.7 tests per minute (measured as 130 mutations completed in 4.1 minutes, as shown in Table 3), with an average test time of 1.9 seconds per mutation. Through iterative refinement guided by Mirabelle feedback, the oracle achieved perfect alignment (zero false positives) on the 130-mutation training set.
...
Our validation methodology provides confidence in this interpretation. The perfect alignment between our trained oracle and Mirabelle (Isabelle's official testing tool) on all 130 test cases indicates that we are not missing bugs due to oracle misalignment. The oracle was iteratively refined using Mirabelle as ground truth until achieving complete agreement on the dataset. Both the trained oracle and Mirabelle reported no integration bugs on these 130 mutants, though this observation is limited to the tested mutation space and the oracle's training set.
...
Our work makes several contributions to fuzzing and testing methodology that extend beyond the specific domain of theorem prover testing. The two-phase workflow we developed represents a practical approach to the oracle accuracy problem in fuzzing. Traditional fuzzing tools typically rely solely on their own oracles for bug detection, with limited external validation of findings. In contrast, our approach uses an official tool (Mirabelle) as ground truth to train a fast custom oracle through iterative refinement. This combination captures the best of both approaches: the custom oracle enables rapid iteration and high throughput, while alignment with official tool judgments ensures accuracy. This methodology is particularly valuable in domains where authoritative validation tools exist but are too slow for direct use in large-scale fuzzing campaigns.

The techniques we employed for bug detection represent practical contributions applicable to building test oracles in any domain. Our pattern-based filtering approach incorporates several key insights. First, test oracles must consider the overall execution context rather than relying solely on keyword matching in output streams. The presence of error-related keywords does not necessarily indicate failure if the overall execution succeeded---modern software tools produce verbose output including warnings and status updates that may contain alarming keywords despite representing normal operation. Second, distinguishing between warnings and genuine errors requires understanding the specific conventions and signaling mechanisms of the tested system. In Isabelle's case, critical errors are marked with specific prefixes while warnings use different formatting. Third, separation of concerns is crucial: distinguishing theory-level errors from integration bugs prevents misclassification of user input errors as system bugs. Finally, alignment with official tool semantics through iterative refinement ensures that the oracle's notion of success and failure matches the authoritative definition.

Our oracle design provides a template for other researchers building test oracles through supervised learning. The key is to employ pattern-based filtering refined against official reference tools. In domains where official validation tools exist (whether automated tools like Mirabelle, manual expert review, or cross-validation with multiple independent oracles), they should be leveraged as ground truth for oracle training. The filtering techniques we demonstrate---examining success indicators, distinguishing error severities, filtering input errors, and identifying interface-specific issues---are generalizable to other testing contexts where output interpretation is complex.

\subsection{Practical Impact}

Our work provides immediate practical value to multiple stakeholder communities. 

\textbf{For Isabelle developers and maintainers}, our extensive testing campaign provides \textbf{fuzzing-based evidence} that the integration layer is stable \textbf{within the tested mutation space}. The fact that all 130 diverse mutants were handled without integration failures, validated by official Mirabelle testing, gives confidence that the interface handles these tested edge cases correctly. Additionally, our identification of problematic prover configurations (specifically, which provers timeout or error on which types of problems) can inform default prover selection strategies in future Isabelle releases. Perhaps most valuably, our framework provides a ready-made regression testing infrastructure that can be deployed whenever Sledgehammer's encoding strategies or integration logic are modified, ensuring that changes do not introduce new bugs.
...
\textbf{For the broader research community}, our work contributes both methodology and empirical insights. The two-phase workflow we developed provides a template for training test oracles using official validation tools as ground truth. The perfect alignment achieved on our training set demonstrates the viability of supervised oracle training in domains where official tools exist. Our AST-based mutation strategies for logical formulas represent a novel application of grammar-aware fuzzing to theorem proving, and could be adapted to other formal methods tools. The empirical finding that Sledgehammer's integration is stable while underlying provers have numerous performance issues challenges assumptions about where reliability problems occur in proof assistant ecosystems.

\subsection{Limitations and Future Work}

Several limitations of our current work suggest promising directions for future research.

\textbf{Oracle Generalization}: Our oracle achieved 0\% false positive rate on the 130 mutations used during development and refinement. While this demonstrates successful alignment with Mirabelle on this dataset, we cannot claim generalization to unseen mutations. The oracle patterns were iteratively refined based on Mirabelle feedback on these 130 cases, essentially treating them as a training set. Evaluation on an independent test set would be necessary to assess generalization capability. The observed 0\% false positive rate should be interpreted as training set performance rather than validation set performance, and future work should evaluate oracle accuracy on held-out data to measure generalization.
...
\textbf{Campaign Scale}: The scale of our fuzzing campaign, while sufficient to demonstrate framework capabilities, represents only a fraction of the input space that could be explored. Our 130 mutations provide good coverage of the implemented mutation operators and seed theory diversity, but campaigns with thousands or tens of thousands of mutations could potentially reveal rarer edge cases that only manifest under unusual combinations of features. The relatively small number of mutations was constrained by the exploratory nature of this work and the time required for manual analysis of results, but automated analysis pipelines could enable much larger-scale testing.
...
Third, we developed a pattern-based bug detection oracle refined through iterative alignment with Mirabelle as ground truth. The oracle employs success indicator checking, critical error detection, theory error filtering, and interface issue identification. Through supervised training guided by Mirabelle feedback, the oracle achieved perfect alignment (0\% false positives) on the 130-mutation training set. This demonstrates that building reliable test oracles benefits from alignment with official reference tools through iterative refinement.

Fourth, our mutation-based evaluation establishes a \textbf{lower bound on the reliability} of the Sledgehammer interface on the tested mutation space. By withstanding 130 valid structural mutations without a single failure---validated against Mirabelle---we provide evidence that, in our evaluation, instability appears confined to the external provers (519 bugs) rather than the integration logic. This finding helps direct future testing efforts toward prover performance rather than interface reliability.

From a methodological perspective, we contribute several generalizable techniques. Our two-phase workflow, using Mirabelle as ground truth for training a fast screening oracle, represents a practical approach to the oracle accuracy problem in fuzzing research. The pattern-based filtering techniques we employed---examining success markers, distinguishing critical errors from warnings, filtering theory-level errors, and focusing on interface-specific issues---are applicable to building oracles in other testing contexts. Our AST-based mutation strategies for logical formulas extend grammar-aware fuzzing to the theorem proving domain. The oracle design, refined through alignment with official tools, provides a template that other researchers can follow when developing test oracles for domains where official reference tools are available for supervised training.
